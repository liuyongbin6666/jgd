...
namespace GMVC.Core
{
    ...
    public class UiBuilder
    {
        ...
        /// <param name="buildAction">生成函数<see cref="GameObject"/>和<see cref="IView"/>为传入物件</param>
        /// <param name="rootCanvas"></param>
        async Task<T> Build<T>(string resName, Func<GameObject, IView, T> buildAction, bool rootCanvas = true) =>
        ...
        async Task<T> Build<T>(string resName, Transform parent, Func<GameObject, T> buildAction)
        {
            var obj = await Res.InstantiateAsync(resName, parent);
